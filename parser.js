 ...
 */
//
// export function buildSongString(song) {
//   const title = song.title.replace(/^(The|A) (.+)/, '$2, $1');
//   const composer = song.composer.split(/(\s+)/).length === 3
//     ? song.composer.split(/(\s+)/).reverse().join('')
//     : song.composer;
//
//   const music = serialize(song.cells);
//   const scrambled = scramble(music);
//   const musicPart = `1r34LbKcu7${scrambled}`;
//
//   return [
//     title,
//     composer,
...
// }
//
// export function buildPlaylistURL(playlist) {
//   const songsEncoded = playlist.songs.map(buildSongString).join('===');
//   const full = songsEncoded + '===' + (playlist.name || '');
//   return `irealb://${encodeURIComponent(full)}`;
// }
//
// function scramble(s) {
//   // Substitutions inverses
//   s = s.replace(/ \|/g, 'LZ').replace(/\| x/g, 'Kcl').replace(/ {3}/g, 'XyQ');
//
//   let r = '', p;
//   while (s.length > 51) {
//     p = s.substring(0, 50);
...
// }
//
// function scramble50(s) {
//   const arr = s.split('');
//   for (let i = 0; i < 5; i++) {
//     [arr[i], arr[49 - i]] = [arr[49 - i], arr[i]];
//   }
//   for (let i = 10; i < 24; i++) {
//     [arr[i], arr[49 - i]] = [arr[49 - i], arr[i]];
//   }
...
//
//     // Spacer
//     parts.push('Y'.repeat(cell.spacer));
//
//     // Annotations
//     parts.push(...cell.annots);
//
//     // Commentaires
//     parts.push(...cell.comments.map(c => `<${c}>`));
//
//     // Barres
//     parts.push(cell.bars);
//
//     // Accord
//     if (cell.chord) {
//       let chordStr = cell.chord.note + (cell.chord.modifiers || '');
//       if (cell.chord.over) chordStr += '/' + cell.chord.over.note + (cell.chord.over.modifiers || '');
//       if (cell.chord.alternate) chordStr += `(${cell.chord.alternate.note}${cell.chord.alternate.modifiers || ''})`;
//       parts.push(chordStr);
//     }
//
//     return parts.join('');
    ...
    this.songs = parts
      ...
      .reduce((songs, song) => {
        if (songs.length > 0) {
          ...
          // The `window.diff` module compares two titles and returns a list of similarities and differences.
          // We expect the first window.diff to be a similarity, followed by differences that are only numeric.
          // When we find a multi-part song, we just concatenate the cells into the first part.
          const diffs = window.diff(songs[songs.length-1].title, song.title);
...
export class Song {
  constructor(ireal, oldFormat = false) {
    ...
    if (oldFormat) {
      ...
      console.log("-----");
      console.log(`scrumbled: ${this.title}: "${music[1]}"`);
      const unscrumbled = unscramble(music[1]);
      console.log(`UNscrumbled: ${this.title}: "${unscrumbled}"`);
      const rescrumbled = scramble(unscrumbled);
      console.log(`REscrumbled: ${this.title}: "${rescrumbled}"`);
